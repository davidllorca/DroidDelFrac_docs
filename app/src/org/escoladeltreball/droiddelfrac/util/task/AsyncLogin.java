 ...
 */
public class AsyncLogin extends AsyncTask<String, Void, User> {
	...
	protected void onPostExecute(User user) {
		if (user != null) {
			String email = ((LoginActivity) activity).getEmail();
			String password = ((LoginActivity) activity).getPassword();
			...
			((LoginActivity) activity).savePreferences("mailUser", email);
			// Si el checkbox isChecked guardamos usuario y contaseña en
			// SharedPreferences
			if (((LoginActivity) activity).getCheckBox().isChecked()) {
				((LoginActivity) activity).saveKeepLogged(true);
//				((LoginActivity) activity).savePreferences("mailUser", email);
				((LoginActivity) activity).savePreferences("password", password);
				((LoginActivity) activity).savePreferences("nameUser",nameUser);
				((LoginActivity) activity).savePreferences("idUser", idUser);
			} else {
				((LoginActivity) activity).saveKeepLogged(false);
			}
			...
			((LoginActivity) activity).setTextPasswordEditText("");
		}
