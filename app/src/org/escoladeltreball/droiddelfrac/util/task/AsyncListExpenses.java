...
/**
 * Realiza una llamada al servidor para solicitar los gastos de un evento
 * determinado. Devuelve un objeto ExpenseManager con todos los gastos unitarios
 * del evento.
 * 
...
public class AsyncListExpenses extends AsyncTask<String, Void, ListEvExpenses> {

	private static final String TAG = "AsyncListExpenses";
	private Context context;

	public AsyncListExpenses(Context context) {
		this.context = context;
	}
	
	@Override
	protected ListEvExpenses doInBackground(String[] url) {
	...
	private ListEvExpenses handleResult(String result) {
		ListEvExpenses listExpenses = null;
		...
		if (!result.equals("protocol error") && !result.equals("IO error")) {
			try {
				...
				if (code.equals("200")) {
					...
					String listString = GetData.getStringFromJSON("data", "expense", result);
					Log.i("RESULT EventUser", listString);
					listExpenses = new Gson().fromJson(listString, ListEvExpenses.class);
